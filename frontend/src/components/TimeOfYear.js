    ...
    return firebase
      ...
      .then(function (querySnapshot) {
        querySnapshot.forEach(function (doc) {
          ...
          if (newData.timeElapsedInAYear > 0) {
            setYourData((arr) => {
              return [...arr, newData];
            });
            maximumArray.push(newData.speed);
            selfAssessArray.push(newData.timeElapsedInAYear);
            let newObject = {};
            newObject.x = newData.timeElapsedInAYear;
            newObject.y = newData.speed;
            newObject.time = newData.currentTime;
            newObject.date = newData.currentDate;
        ...
        if (slopeRound > 0) {
          setExplanation(
            "Because the Blue Line has a positive slope (" +
              slopeRound +
              ") the data could be suggesting a positive correlation between the time it takes you to react and the time in the Year."
          );
        } else if (slopeRound < 0) {
          setExplanation(
            "Because the Blue Line has a negative slope (" +
              slopeRound +
              ") the data could be suggesting a negative correlation between the time it takes you to react and the time in the Year."
          );
        } else {
          setExplanation(
            "Because the Blue Line has a zero slope (" +
              slopeRound +
              ") the data could be suggesting that there is no correlation between the time it takes you to react and the time in the Year."
          );
        }
      })
    ...
    <SafeAreaView style={styles.container2}>
      <View style={styles.center}>
        <TouchableOpacity style={styles.middle}>
          <Text style={styles.text2}>Time Of Year</Text>
